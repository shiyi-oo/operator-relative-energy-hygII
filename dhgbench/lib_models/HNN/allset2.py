...
from lib_models.HNN.utils import glorot
import math
...
class HalfNLHconv(MessagePassing):
    def __init__(self, in_dim, hid_dim, out_dim, num_layers, dropout, Normalization='bn',
                 InputNorm=False, heads=1, attention=True):
...
class SetGNNII(nn.Module):
    def __init__(self, num_features, num_targets, args, norm=None):
        ...
        self.restart_alpha = args.restart_alpha
        self.residual_linears = nn.ModuleList([
            nn.Linear(args.MLP_hidden, args.MLP_hidden, bias=False)
            for _ in range(self.All_num_layers)
        ])
        self.lin_input = nn.Linear(num_features, args.MLP_hidden)
    ...
    def forward(self, data):
        ...
        restart_alpha = self.restart_alpha
        lamda = self.lamda
        if self.GPR:
            xs = []
            x = F.relu(self.MLP(x))
            ...
            for i, _ in enumerate(self.V2EConvs):
                ...
                Xi = (1 - restart_alpha) * x + restart_alpha * x0
                x = (1 - beta) * Xi + beta * self.residual_linears[i](Xi)
