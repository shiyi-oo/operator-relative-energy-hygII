...
from lib_dataset.load_other_datasets import load_dataset, load_cornell_dataset, load_LE_dataset 
...
class dataset_Hypergraph(InMemoryDataset):
    ...
    def __init__(self, root = '../data/pyg_data/hypergraph_dataset_updated/', name = None, 
        ...
        existing_dataset = _single_datasets_
    ...
    def download(self):
        for name in self.raw_file_names:
            ...
            if not osp.isfile(p2f):
                ...
                if self.name in ('cora', 'citeseer', 'pokec'):
                    tmp_data = load_dataset(path=self.p2raw, dataset=self.name,
                                            train_percent=self._train_percent)
                elif self.name == 'house-committees-100':
                    if self.feature_noise is None:
                        raise ValueError('House-Committee requires feature_noise')
                    tmp_data = load_cornell_dataset(path=self.p2raw,
                        dataset='house-committees', feature_dim=100,
                        feature_noise=self.feature_noise, train_percent=self._train_percent)
                else:
                    tmp_data = load_LE_dataset(path=self.p2raw, dataset=self.name,
                                               train_percent=self._train_percent)
